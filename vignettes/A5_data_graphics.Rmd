...
```{r doit, echo=FALSE}
make_hist = function(site="Prostate", binwidth=5) {
 tt = littab |> filter(`Cancer Sites` == site)
 suppressMessages({
  suppressWarnings({
...
make_boxplot1 = function(site="Prostate", MSA.in="Baton Rouge, LA") {
 tt = littab |> filter(MSA == MSA.in) 
 suppressMessages({
  suppressWarnings({
...
make_pair = function(MSA1="Baton Rouge, LA", MSA2="Tucson, AZ", type="boxplot") {
 tt = littab |> filter(MSA %in% c(MSA1, MSA2)) |> filter(`Cancer Sites` != "All Invasive Cancer Sites Combined")
...
```{r lkhist}
make_hist(site="Prostate", binwidth=5)
```
...
```{r lkhist2}
make_hist(site="Prostate", binwidth=2)
```
...
```{r lk3}
ggplotly(make_pair(type="beeswarm"))
```
