...
![Lymphatic cancers](lymphlogic.png)
...
![Details of lymphatic cancers](lymExpanded.png)

If we "clear" the selection, we get a complete overview of
all studies and cancer types.

![Selection cleared](clearGDC.png)

Exercise: After clearing the selection, hover over the
...
```{r dopl1,echo=FALSE}
mkid = SEER_2018 |> filter(CancerSite == "Kidney and Renal Pelvis" & Sex == "Male") 
fcol = SEER_2018 |> filter(CancerSite == "Colon and Rectum" & Sex == "Female") 
with(mkid |> filter(Race=="All Races"), plot(rate.2.2018~Year, ylab="Incidence per 100000", xlab="Year",
    main="SEER Kidney and Renal Pelvis, Male"))
with(fcol |> filter(Race=="All Races"), plot(rate.2.2018~Year, ylab="Incidence per 100000", xlab="Year",
...
<!--
dim(mkid)
with(mkid |> filter(Race=="All Races" & Year >= 2013), lm(rate.2.2018~Year))
with(mkid |> filter(Race=="All Races" & Year >= 2013), plot(rate.2.2018~Year))
6/17
17+.35*17
with(mkid |> filter(Race=="All Races"), plot(rate.2.2018~I(Year-2000)))
with(mkid |> filter(Race=="All Races"), lm(rate.2.2018~I(Year-2000)))
.34/17
with(mpanc |> filter(Race=="All Races"), lm(rate.2.2018~I(Year-2000)))
.125/12.89
head(mkid)
head(mpanc)
SEER_2018 |> filter(CancerSite == "Leukemia" & Sex == "Male")  -> mleuk
head(mleuk)
SEER_2018 |> filter(CancerSite == "Lung and Bronchus" & Sex == "Male")  -> mlung
head(mlung)
with(mlung |> filter(Race=="All Races"), lm(rate.2.2018~I(Year-2000)))
with(mlung |> filter(Race=="All Races"), plot(rate.2.2018~I(Year-2000)))
-1.8.88.7
-1.8/88.7
with(mlung |> filter(Race=="All Races" & Year >=2013), plot(rate.2.2018~I(Year-2000)))
with(mlung |> filter(Race=="All Races" & Year >=2013), lm(rate.2.2018~I(Year-2000)))
-2.06/91.5
-->
...
![Oncotree snippet](oncotreeSNIP.jpg)
